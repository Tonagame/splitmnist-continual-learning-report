...
METHODS = [
    "None", "EWC", "LwF", "A-GEM", "Separate Networks", "H&T",
    "H&T + Fourier", "H&T + ASW", "H&T + Fourier + ASW", "Joint",
]
...
COMMANDS = {
    ...
    "Separate Networks": f'& "{PYTHON}" main.py {COMMON_MAIN} --separate-networks --eval-history-method="Separate Networks"',
    ...
    "Joint": f'& "{PYTHON}" main.py --experiment=splitMNIST --scenario=task --contexts=5 --iters=10000 --batch=128 --acc-n=1024 --acc-log=100 --time --no-save --budget=100 --joint --eval-history-method=Joint',
}
LOGS = {
    "None": "splitMNIST_task_2000_none.log",
    "EWC": "splitMNIST_task_2000_ewc.log",
    "LwF": "splitMNIST_task_2000_lwf.log",
    "A-GEM": "splitMNIST_task_2000_agem.log",
    "Separate Networks": "splitMNIST_task_2000_separate_networks.log",
    "H&T": "splitMNIST_task_2000_h_and_t.log",
    "H&T + Fourier": "splitMNIST_task_2000_h_and_t_fourier.log",
    "H&T + ASW": "splitMNIST_task_2000_h_and_t_asw.log",
    "H&T + Fourier + ASW": "splitMNIST_task_2000_h_and_t_fourier_asw.log",
    "Joint": "splitMNIST_task_2000_joint.log",
}
...
def find_result_files(results):
    """Locate Phase 3 final-accuracy files for each method."""

    return {
        "None": latest(results.glob("acc-splitMNIST5-task--F-784x400x400_c10--i2000-lr0.001-b128-adam.txt")),
        "EWC": latest(results.glob("acc-splitMNIST5-task--F-784x400x400_c10--i2000-lr0.001-b128-adam--PReg*-offline.txt")),
        "LwF": latest(results.glob("acc-splitMNIST5-task--F-784x400x400_c10--i2000-lr0.001-b128-adam--current-KD2.0.txt")),
        "A-GEM": latest(results.glob("acc-splitMNIST5-task--F-784x400x400_c10--i2000-lr0.001-b128-adam--buffer-A-GEM*.txt")),
        "Separate Networks": latest(results.glob("acc-splitMNIST5-task--SepNets-F-784x100x100_c2--i2000-lr0.001-b128-adam.txt")),
        "H&T": latest(results.glob("acc-splitMNIST5-task--H&T--i2000-b128-bud100-kd1.0-feat1.0.txt")),
        "H&T + Fourier": latest(results.glob("acc-splitMNIST5-task--H&T-Fourier--i2000-b128-bud100-kd1.0-feat1.0-fft0.1.txt")),
        "H&T + ASW": latest(results.glob("acc-splitMNIST5-task--H&T-ASW--i2000-b128-bud100-kd1.0-feat0.5-asw0.5-2.0-eps1e-08.txt")),
        "H&T + Fourier + ASW": latest(results.glob("acc-splitMNIST5-task--H&T-Fourier-ASW--i2000-b128-bud100-kd1.0-feat0.5-fft0.05-asw0.5-2.0-eps1e-08.txt")),
        "Joint": latest(results.glob("acc-splitMNIST5-Joint-task--F-784x400x400_c10--i10000-lr0.001-b128-adam.txt")),
    }


def find_time_files(results):
    """Locate repository runtime files for classic Phase 3 methods."""

    return {
        "None": latest(results.glob("time-splitMNIST5-task--F-784x400x400_c10--i2000-lr0.001-b128-adam.txt")),
        "EWC": latest(results.glob("time-splitMNIST5-task--F-784x400x400_c10--i2000-lr0.001-b128-adam--PReg*-offline.txt")),
        "LwF": latest(results.glob("time-splitMNIST5-task--F-784x400x400_c10--i2000-lr0.001-b128-adam--current-KD2.0.txt")),
        "A-GEM": latest(results.glob("time-splitMNIST5-task--F-784x400x400_c10--i2000-lr0.001-b128-adam--buffer-A-GEM*.txt")),
        "Separate Networks": latest(results.glob("time-splitMNIST5-task--SepNets-F-784x100x100_c2--i2000-lr0.001-b128-adam.txt")),
        "Joint": latest(results.glob("time-splitMNIST5-Joint-task--F-784x400x400_c10--i10000-lr0.001-b128-adam.txt")),
    }
...
def load_asw_stats(results, method):
    ...
    pattern = {
        "H&T + ASW": "metrics-splitMNIST5-task--H&T-ASW--i2000-b128-bud100-kd1.0-feat0.5-asw0.5-2.0-eps1e-08.csv",
        "H&T + Fourier + ASW": "metrics-splitMNIST5-task--H&T-Fourier-ASW--i2000-b128-bud100-kd1.0-feat0.5-fft0.05-asw0.5-2.0-eps1e-08.csv",
    }.get(method)
    if not pattern:
        return {}
...
def plot_final(rows, out):
    """Create the Phase 3 final-accuracy bar chart."""

    methods = [r["method"] for r in rows]
    vals = [float(r["final_accuracy"]) if r["final_accuracy"] else math.nan for r in rows]
    colors = ["#7a8599", "#2f80ed", "#27ae60", "#f2994a", "#8b5cf6",
    ...
    for bar, val in zip(bars, vals):
        if math.isnan(val):
            ...
            ax.text(bar.get_x() + bar.get_width() / 2, val + 0.004, f"{val:.3f}", ha="center", va="bottom", fontsize=8)
    ax.set_ylim(0, 1.03)
    ax.set_ylabel("Final accuracy")
    ax.set_title("Split MNIST Task-CL 2000: Final Accuracy")
...
def plot_learning_curve(curve_path, graph_path, context_graph_path):
    """Create raw and context-aggregated Phase 3 learning-curve graphs."""
    ...
    with curve_path.open(newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            by_method.setdefault(row["method"], []).append(
                (int(float(row["iteration"])), float(row["accuracy"]), int(float(row["context"])))
            )
    ...
    ax.set_ylabel("Average Task-CL accuracy")
    ax.set_title("Split MNIST Task-CL 2000: Learning Curve")
    ...
    ax.set_title("Split MNIST Task-CL 2000: Context-Aggregated Learning Curve")
...
def main():
    """Aggregate Phase 3 outputs into summary.csv, graphs, and a report."""
    ...
    plot_final(rows, results / "splitMNIST_task_2000_final_accuracy.png")
    plot_learning_curve(
        results / "learning_curve.csv",
        results / "splitMNIST_task_2000_learning_curve.png",
        results / "splitMNIST_task_2000_learning_curve_by_context.png",
    )
    ...
    lines += [
        ...
        f"Separate Networks performed as expected in Task-CL: {'yes' if acc.get('Separate Networks', 0) >= acc.get('None', 1) else 'mixed/no'}",
    ...
    lines += [
        ...
        f"- final graph: {results / 'splitMNIST_task_2000_final_accuracy.png'}",
        f"- learning curve: {results / 'splitMNIST_task_2000_learning_curve.png'}",
        f"- context curve: {results / 'splitMNIST_task_2000_learning_curve_by_context.png'}",
        f"- logs: {results / 'logs'}",
    ]
    (results / "PHASE3_REPORT.md").write_text("\n".join(lines), encoding="utf-8")
    print(results / "summary.csv")
    print(results / "PHASE3_REPORT.md")
